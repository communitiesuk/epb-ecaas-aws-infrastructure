...
resource "aws_api_gateway_method" "frontend_app_root" {
  rest_api_id = aws_api_gateway_rest_api.ecaas_frontend.id
  resource_id = aws_api_gateway_rest_api.ecaas_frontend.root_resource_id
  http_method   = "ANY"
  authorization = "NONE"
}
...
resource "aws_api_gateway_integration" "frontend_app_root" {
  rest_api_id             = aws_api_gateway_rest_api.ecaas_frontend.id
  resource_id             = aws_api_gateway_method.frontend_app_root.resource_id
  http_method             = aws_api_gateway_method.frontend_app_root.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.front_end_lambda.invoke_arn
}
